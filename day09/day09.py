...
def extrapolate_lines(sensor_file_name, is_reverse=False):
    ...
    with pathlib.Path(sensor_file_name).absolute().open() as f:
        for line in f:
            if not is_reverse:
                output_list.append(calc_next(line))
            else:
                output_list.append(calc_previous(line))
...
def calc_previous(line):
    ...
    while list_of_lists[i].count(0) != len(list_of_lists[i]):
        next_list = []
        j = len(list_of_lists[i]) - 1
        while j >= 1:
            next_list.insert(0, (list_of_lists[i][j] - list_of_lists[i][j-1]))
            j -= 1
        list_of_lists.append(next_list)
        i += 1
    # Append a 0 to the list of zeros for calculations.
    ...
    i = len(list_of_lists) - 2
    while i >= 0:
        list_of_lists[i].insert(
            0, list_of_lists[i][0] - list_of_lists[i+1][0])
        i -= 1
