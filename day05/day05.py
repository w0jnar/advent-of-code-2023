...
def check_almanac(almanac_file_name):
    ...
    def get_location(seed, seed_to_soil_dict, soil_to_fertilizer_dict, fertilizer_to_water_dict,
        ...
        water = get_value_from_almanac_dict(fertilizer_to_water_dict, fertilizer)
        light = get_value_from_almanac_dict(water_to_light_dict, water)
        temperature = get_value_from_almanac_dict(light_to_temperature_dict, light)
        humidity = get_value_from_almanac_dict(temperature_to_humidity_dict, temperature)
...
def check_almanac_reverse(almanac_file_name):
    ...
    while i < len(temp_seed_list):
        seed_range_list.append(range(temp_seed_list[i], (temp_seed_list[i] + temp_seed_list[i+1])))
        i += 2
    ...
    while not is_location_found:
        humidity = get_key_from_almanac_dict(humidity_to_location_dict, location)
        temperature = get_key_from_almanac_dict(temperature_to_humidity_dict, humidity)
        light = get_key_from_almanac_dict(light_to_temperature_dict, temperature)
...
def build_almanac_dicts(file_list):
    ...
    while line_number < len(file_list):
        if 'map' in file_list[line_number]:
            line_number += build_almanac_dict(file_list, line_number, eval(f'{file_list[line_number].split()[0].replace('-', '_')}_dict'))
        line_number += 1
...
def build_almanac_dict(file_list, index, dictionary):
    ...
    while i < len(file_list) and len(file_list[i]) > 1:
        ...
        dictionary[f'{source_range},{source_range + range_length - 1}'] = f'{destination_range},{destination_range + range_length - 1}'
        i += 1
    return i - index
